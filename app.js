...
const APP_REPOSITORY = 'https://github.com/x-inu/cloud-mail'
const DOCS_URL = 'https://doc.xinu.my.id'
...
const pages = {
  about: {
    ...
    sections: [
      ['overview', 'System Overview', `<p>Xinu Mail receives messages through Cloudflare Email Routing and runs its API and web interface on Cloudflare Workers. D1 stores structured data, KV stores settings and lightweight objects, and R2 or an S3-compatible service can store attachments.</p><p>Use the official <a href="${APP_REPOSITORY}">Xinu Mail repository</a> for deployment source and <a href="${DOCS_URL}">doc.xinu.my.id</a> for current documentation.</p>`],
  ...
  dashboard: {
    ...
    sections: [
      ['account-preparation', 'Account Preparation', '<p>Register a Cloudflare account at <a href="https://dash.cloudflare.com/">dash.cloudflare.com</a> and add the domain you will use for Xinu Mail.</p>' + image('1.Ceip4Aks.png', 'Cloudflare account dashboard showing an active domain')],
      ['create-project', 'Create the Project', `<ol><li>Fork or clone the official <a href="${APP_REPOSITORY}">Xinu Mail repository</a>.</li><li>Create a Worker project.</li><li>Choose <strong>Import from GitHub</strong>.</li><li>Select <code>x-inu/cloud-mail</code>, set the root directory to <code>mail-worker</code>, and deploy.</li></ol>` + image('0.BgZTtVBn.png', 'Fork the application repository on GitHub') + image('1.C_OKFTJq.png', 'Create a Worker project in Cloudflare') + image('2.yrULln52.png', 'Import a Worker project from GitHub') + image('3.BlAKorP8.png', 'Set the Worker root directory')],
      ...
      ['email-routing', 'Configure Email Routing', '<ol><li>Open your domain in Cloudflare.</li><li>Go to <strong>Email → Email Routing → Routing Rules</strong>.</li><li>Create a catch-all rule that sends incoming messages to the Xinu Mail Worker.</li><li>Confirm the rule is active before testing delivery.</li></ol>' + image('6.B8ZEA4eb.png', 'Cloudflare Email Routing setup') + image('7.Bm0YMP0b.png', 'Cloudflare Email Routing destination configuration') + image('8.jfYabKoj.png', 'Cloudflare Email Routing catch-all Worker rule')],
      ['initialize', 'Initialize and Sign In', '<ol><li>Open <code>https://&lt;worker-domain&gt;/api/init/&lt;jwt-secret&gt;</code> in a trusted browser session.</li><li>Remove that URL from browser history because it contains the secret.</li><li>Open the Worker domain, register the administrator account, and sign in to Xinu Mail.</li></ol>' + image('9.UpdVuA4A.png', 'Application sign-in screen')]
    ]
  },
  action: {
    ...
    sections: [
      ...
      ['secrets', 'Configure Repository Secrets', table(['Secret', 'Required', 'Description'], [['CLOUDFLARE_API_TOKEN', 'Yes', 'Cloudflare API token'], ['CLOUDFLARE_ACCOUNT_ID', 'Yes', 'Cloudflare account ID'], ['CUSTOM_DOMAIN', 'Yes', 'Xinu Mail custom domain'], ['DOMAIN', 'Yes', 'Email domain array'], ['ADMIN', 'Yes', 'Administrator address'], ['JWT_SECRET', 'Yes', 'Long random signing secret'], ['NAME', 'No', 'Worker project name'], ['D1_DATABASE_ID', 'No', 'Existing D1 database ID'], ['KV_NAMESPACE_ID', 'No', 'Existing KV namespace ID']]) + image('9.BSzKTXoh.png', 'GitHub repository Actions secrets list')],
      ['deploy', 'Run Deployment', `<ol><li>Open the Actions tab in <a href="${APP_REPOSITORY}">x-inu/cloud-mail</a> or your private fork.</li><li>Run the Xinu Mail deployment workflow.</li><li>Wait for all workflow steps to complete.</li><li>Configure Email Routing and open the custom domain.</li></ol>` + image('10.D1da10Ql.png', 'Run the GitHub Actions workflow') + image('11.BgS0gJ4t.png', 'Completed GitHub Actions deployment workflow')]
    ]
  },
  command: {
    title: 'Command Line Deployment',
    sections: [
      ['prerequisites', 'Prerequisites', '<ul><li>Node.js 22 or newer</li><li>pnpm</li><li>A Cloudflare account with an active domain</li></ul>' + code('shell', 'git clone https://github.com/x-inu/cloud-mail\ncd cloud-mail/mail-worker\npnpm install')],
      ['configuration', 'Worker Configuration', '<p>Configure D1, KV, optional R2 or Email Service bindings, Workers AI, static assets, cron triggers, and build command in <code>mail-worker/wrangler.toml</code>. Keep the Worker and database names aligned with the tutorial screenshots.</p>' + code('toml', 'name = "cloud-mail"\nmain = "src/index.js"\ncompatibility_date = "2025-06-04"\nkeep_vars = true\n\n[[d1_databases]]\nbinding = "db"\ndatabase_name = "email"\ndatabase_id = "<D1_DATABASE_ID>"\n\n[[kv_namespaces]]\nbinding = "kv"\nid = "<KV_NAMESPACE_ID>"\n\n[ai]\nbinding = "ai"\n\n[assets]\nbinding = "assets"\ndirectory = "./dist"\nnot_found_handling = "single-page-application"\nrun_worker_first = true\n\n[triggers]\ncrons = ["*/30 * * * *", "0 16 * * *"]\n\n[build]\ncommand = "pnpm --prefix ../mail-vue install && pnpm --prefix ../mail-vue run build"')],
  ...
  sending: {
    ...
    sections: [['resend', 'Configure Resend', '<ol><li>Register at <a href="https://resend.com/">Resend</a>, add your Xinu Mail domain, and verify its DNS records.</li><li>Create an API key and store it in Xinu Mail System Settings.</li><li>Add <code>https://&lt;xinu-mail-domain&gt;/api/webhooks</code> as the status callback.</li><li>Select the delivery events you need.</li><li>Complete the sending configuration in Xinu Mail.</li></ol>' + image('1.DwYi9syY.png', 'Resend domain verification screen') + image('2.FAyxtFE4.png', 'Resend webhook callback configuration') + image('4.B6iEcvcq.png', 'Resend webhook event selection') + image('5.DXbq-DgO.png', 'Application sending configuration') + notice('Credential handling', 'Never expose the complete Resend API key in screenshots, logs, or source control.')]]
  },
  storage: {
    title: 'Object Storage',
    intro: 'Attachments use KV by default. R2 or another S3-compatible provider can be configured for object storage.',
    sections: [['r2', 'Configure R2', '<ol><li>Create an R2 bucket for Xinu Mail attachments.</li><li>Configure a custom domain under your own Xinu domain if public object delivery is required.</li><li>Add the optional <code>r2</code> binding or <code>R2_BUCKET_NAME</code> Action value.</li><li>Complete object storage settings in Xinu Mail.</li></ol>' + image('1.DDX40T9J.png', 'Create a Cloudflare R2 bucket') + image('2.wkBqZxnz.png', 'Configure an R2 custom domain') + image('3.BH3jpxf4.png', 'Application object storage configuration')]]
  },
  turnstile: {
    title: 'Turnstile',
    sections: [['widget', 'Configure Human Verification', '<ol><li>Create a Turnstile widget for the Xinu Mail hostname.</li><li>Copy its site key and secret key.</li><li>Open Xinu Mail System Settings and configure human verification.</li></ol>' + image('1.DpI-QwiV.png', 'Create a Cloudflare Turnstile widget') + image('3.DOxY6443.png', 'Application Turnstile configuration') + notice('Secret key', 'The site key is public. Treat the secret key as a credential and never include it in screenshots, logs, or source control.')]]
  },
  forward: {
    title: 'Email Forwarding',
    sections: [
      ['telegram', 'Forward to Telegram', '<ol><li>Open the verified <code>@BotFather</code> account.</li><li>Create a bot and store its token securely.</li><li>Send a message to the bot.</li><li>Request <code>https://api.telegram.org/bot&lt;BOT_TOKEN&gt;/getUpdates</code> and copy only the numeric <code>chat.id</code>.</li><li>Configure Telegram forwarding in Xinu Mail.</li></ol>' + image('1.COkzfE5i.png', 'Verified BotFather account in Telegram') + image('3.DdcugslA.png', 'Send an initial message to a Telegram bot') + image('5.BvE5wcey.png', 'Application Telegram forwarding configuration') + notice('Bot token', 'Anyone with the bot token can control the bot. Do not publish the token or the getUpdates response.')],
      ['email', 'Forward to Another Address', '<ol><li>Complete destination address verification in Cloudflare.</li><li>Configure email forwarding in Xinu Mail System Settings.</li></ol>' + image('6.FvUPZDU2.png', 'Cloudflare destination address verification') + image('7.BWZlPlip.png', 'Application email forwarding configuration')]
    ]
  },
  api: {
    title: 'API Reference',
